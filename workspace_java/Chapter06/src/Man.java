...
// -> 멤버변수와 메소드를 내 것처럼 사용 가능!
//상속 관계에 있는 자식 클래스의 객체를 생성할 때, 내부적으로 부모 클래스의 객체도 생성한다.
//부모 클래스의 객체를 생성할 때도 생성자를 호출해서 객체를 만든다.
//이 때, 호출되는 생성자는 매개변수가 없는 생성자를 호출한다.
//그리고, 이렇게 부모클래스의 기본생성자를 호출하는 문법은 숨겨져있다(생략되어 있다).
//생략되어 있는 부모클래스의 생성자 호출 문법은 자식클래스의 생성자 첫줄에 super() 라는 키워드로 생략되어 있다.
