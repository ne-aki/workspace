...
public class UserDetailsServiceImpl implements UserDetailsService {
  ...
  //LoginFilter 클래스의 attemptAuthentication 메서드 안에서 AuthenticationManager가 입력한 id, pw를 전달받으면
  //아래의 loadUserByUsername 메서드를 호출하여 로그인 검증 시작
  //매개변수 username은 입력한 id값을 가지고 있음
  @Override
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    log.info("UserDetailsServiceImpl -> loadUserByUsername() 메서드 실행");
    ...
    //return문에 로그인하려는 id, pw 정보를 가진 CustomUserDetails 객체를 리턴하면 해당 정보를 AuthenticationManager 객체가 가져감
