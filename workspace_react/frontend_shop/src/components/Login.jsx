...
//로그인한 정보는 컴포넌트가 마운트 혹은 새로고침되어더 사라지지 않아야 한다.
//그래서 로그인 정보는 일반 변수나 state변수에 저장하면 안된다.
//클라이언트(웹 브라우저 ex) 크롬, 사파리 등)는 자체적으로 데이터를 저장할 공간을 3개 제공한다.
//이 세 곳에 데이터를 저장하면 페이지가 새로고침되어도 데이터가 사라지지 않는다.
//3개의 공간 : Cookie, sessionStorage, localStorage
//Cookie : 문자열 데이테만 저장 가능
//         저장 가능 크기 : 4kb
//         Spring(서버)과 통신 시 자동으로 데이터가 전달 됨
//         저장 기간을 설정할 수 있음
//sessionStorage, localStaorgae : 문자열 데이터만 저장 가능
//                                저장가능 크기 : 10mb
...
const Login = ({isOpenLogin, onClose}) => {
  ...
  const loginBtn = () => {
    ...
    .then(res => {
      ...
      if(res.data) {
        alert(`${res.data.memId}님, 환영합니다.`);

        //로그인한 ID, 이름, 권한 정보만을 갖는 객체 생성
        const loginInfo = {
        ...
        //sessionStorage에는 문자열 데이터만 젖아 가능하기 때문에 객체 타입의 res.data를 직접 저장할 수는 없다.
        ...
        //로그인한 회원이 일반 유저라면 상품 목록 페이지로 이동
        else {
          //모달 안 input 태그를 초기화
          setLoginData({
            memId : '',
