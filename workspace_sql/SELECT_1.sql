...
#현재쿼리실행(Ctrl + Shift + F9) : 현재 커서가 올라가 있는 쿼리 실행

# 데이터 조회 기본
# 데이터 조회 SQL 문법
# SELECT 조회할 컬럼 FROM 테이블명 [WHERE 조건]; []: 생략가능함
...
#EMP 테이블에서 급여가 300 이상이고, 사번은 1003번 이상인 사원들의 모든 컬럼을 조회
#그리고(AND), 이거나(OR)
SELECT *
FROM emp
WHERE SAL >= 300
...
#EMP 테이블에서 직급이 사원이 아니고, 급여는 300 이상이면서 커미션은 NULL인 직원의 모든 정보 조회
