#다음과 같은 요구사항을 충족하는 구매이력 테이블(SALE_INFO)을 만들어 보자.
#구매 정보 테이블의 컬럼은 구매번호, 구매 상품명, 구매 수량, 상품 가격, 구매자명, 구매일시(년월일시분초)를 가진다.
...
CREATE TABLE SALE_INFO(
	...
	ITEM_NAME VARCHAR(10) UNIQUE NOT NULL,
...
#2. SALE_NUM이 2번 이상인 구매 정보 중에서 구매자명에 '바'가 포함되거나, '이'씨가 구매한 구매정보의 모든 칼럼정보를 조회
...
#3. 구매자가 '김자바', '윤인사', '정자바'인 구매정보 중,
#구매가격이 5000~20000인 구매정보의 구매자명, 구매상품명, 구매가격을 조회
...
#DROP TABLE SALE_INFO; 데이터가 아닌 TABLE 자체를 삭제
...
#모든 상품의 상품가격을 1000원 인상하는 쿼리 작성
COMMIT;
UPDATE sale_info
SET ITEM_PRICE = ITEM_PRICE + 1000;

#상품명, 상품가격, 구매수량, 총 구매 수량 조회
#총 구매 수량 = 상품가격 * 구매 수량
